...
export const AddCredits: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const { centerOffset } = useDialStore();
  ...
  // Handler for clicks on the modal wrapper
  const handleBackdropClick = (e: React.MouseEvent) => {
    // Only close if the click is directly on the backdrop (e.target === e.currentTarget)
    // and not on any of the modal contents
  ...
  if (paymentStatus) {
    return (
      <div 
        ...
        // Don't allow clicking outside to close when payment is processing
        onClick={paymentStatus === 'success' ? handleBackdropClick : undefined}
      >
        <div 
        ...
        >
          <div 
            className="rounded-xl bg-white p-4 shadow-lg dark:bg-gray-800 max-w-md w-full"
